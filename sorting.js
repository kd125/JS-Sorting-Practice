...
const stringsToNumbs = (numbers) => {
  for (let i = 0; i < numbers.length; i++) {
    numString.push(parseInt(numbers[i]));
  }
  return numString;
};

let onlyNumbers = stringsToNumbs(strNums);
// console.log(onlyNumbers);

// With the same numbers, find the sum of the even values
const sumEvens = onlyNumbers.filter((i) => i % 2 === 0);
let evensAdded = sumEvens.reduce((a, b) => a + b, 0);
// console.log(evensAdded);
...
// console.log(atxIdx(onlyNumbers));
...
let onlyWeatherStates = [];
const weatherStates = (array1) => {
  for (let i = 0; i < array1.length; i++) {
    onlyWeatherStates.push(array1[i].weather_state_name);
  }
  let finalArray = [...new Set(onlyWeatherStates)];
...
const idealTemp = (array1) => {
  for (let i = 0; i < array1.length; i++) {
    if (array1[i].min_temp === 15.915) {
      console.log("The object id is:", array1[i].id);
    }
  }
};
